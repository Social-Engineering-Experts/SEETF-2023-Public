...
struct text
{
    ...
    // char footer[8];
}; // HINT: the header and footer prevents overwriting heap metadata with the UAF, preventing typical solve
...
void create()
{
    ...
    puts("What content?");
    read(0, ptr->buffer, buffer_size); // HINT: prevent tcache poisoning
    memcpy(ptr->header, "=======\nmessage:", 16);
...
void output()
{
    ...
    unsigned int buffer_size; // HINT: allows for stack overlap from create()
    struct text *ptr;         // HINT: same here
    ...
    write(1, ptr, buffer_size + NUM_HEADERFOOTER_BYTES); // HINT: unsorted bin libc leak
}
